 ...
 * 헤드 노드가 있는 원형 리스트 add, delete 구현해 보기, 헤드 노드가 없는 원형 리스트 add, delete 구현해 보기
...
class CircularList {
	...
	void Merge(CircularList b, Comparator<SimpleObject3> cc) {
		 ...
		 * 난이도 등급: 최상급
		 * 회원번호에 대하여 a = (3, 5, 7), b = (2,4,8,9)이면 a = (2,3,4,5,7,8,9)가 되도록 구현하는 코드
		 */
		// 헤드 노드를 쓰면 코드를 깔끔하게 작성할 수 있음(first랑 head 다른 것)
		if (b.first == null) {
			return;
		} else {
...
public class train_실습8_04객체원형리스트 {
	enum Menu {
