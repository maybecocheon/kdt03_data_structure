...
public class Chap04_07_StackMatching {

	public static boolean isValid(String s) {
		/*
		 * 첫 번째 방법) Map을 사용하지 않고 Stack만 써서 구현하기 
		 */
		// 두 번째 방법) Map<Character, Character> pairs = *** // 사용 추천 "[ ]"을 map 쌍으로 정의
		...
		for (char ch : s.toCharArray()) {
			if (bracketPairs.containsValue(ch)) { // 여는 괄호
				...
				if (stack.isEmpty() || stack.pop() != bracketPairs.get(ch)) { // pop 한 건 밸류 아닌가? 왜 key랑 똑같다고 할 수 있지?
